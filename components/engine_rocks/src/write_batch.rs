...
/// `RocksWriteBatchVec` is for method `PebbleWrite` of RocksDB, which splits a large WriteBatch
/// into many smaller ones and then any thread could help to deal with these small WriteBatch when it
/// is calling `PebbleWriteCommit` and wait the front writer to finish writing. `PebbleWrite` will perform
