...
impl Config {
    ...
    pub fn validate(&mut self) -> Result<()> {
        ...
        // The adjustment of this value is related to the number of regions, usually 4096 is
        // already a large enough value
        if self.raft_max_inflight_msgs == 0 || self.raft_max_inflight_msgs > 4096 {
            return Err(box_err!(
                "raft max inflight msgs should be greater than 0 and less than or equal to 4096"
            ));
        }
...
impl ConfigManager for RaftstoreConfigManager {
    fn dispatch(
        ...
        if let Some(ConfigValue::Usize(cap)) = change.get("raft_max_inflight_msgs") {
            if let Err(e) = self
                .scheduler
                .schedule(RefreshConfigTask::AdjustMaxInflightMsgs(*cap))
            {
                error!("raftstore configuration manager schedule adjust max inflight msgs work failed"; "err" => ?e);
            }
        }
        info!(
